<template>
  <view class="container">
    <unicloud-db ref="udb" v-slot:default="{data, loading, error, options}" :options="options" collection="opendb-contacts,opendb-nation-china" field="username,gender,mobile,email,comment,hobby,nation{name as text},address{name as text}" :where="queryWhere" :getone="true" :manual="true">
      ...
      <view v-else-if="data">
        ...
        <view>
          <text>民族</text>
          <text>{{data.nation && data.nation[0] && data.nation[0].name}}</text>
        </view>
        <view>
          <text>地址</text>
